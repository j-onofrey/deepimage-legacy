...
def create_model(input_data, 
    ...
    with tf.variable_scope(name) as scope:
        ...
        pad_size = num_convs_per_layer
        print('***** Setting layer pad size = ',pad_size)
        ...
        for clayer in range(1,num_conv_layers+1):
            ...
            with tf.variable_scope("conv_"+cname) as scope:
                ...
                for i in range(1,num_convs_per_layer):
                    ...
                    print('***** Adding batch normalization')
                    bn = tf.layers.batch_normalization(inputs=conv,name='bn_'+cname+'_'+conv_name)
                    relu = tf.nn.relu(bn,name='relu_'+cname+'_'+conv_name)
                    print('***** Feature shape: '+str(relu.get_shape()))
        ...
        while (dlayer>0):
            ...
            with tf.variable_scope("conv_transpose_"+dname):
                ...
                # NB: TF has a bug with conv3d_transpose where you currently have to turn the bias off
                ...
                print('***** Adding batch normalization')
                bn = tf.layers.batch_normalization(inputs=conv,name='bn_'+dname+'_1')
                relu = tf.nn.relu(bn,name='relu_'+dname+'_1')
                print('***** Feature shape: '+str(relu.get_shape()))
                ...
                for i in range(1,num_convs_per_layer):
                    ...
                    print('***** Adding batch normalization')
                    bn = tf.layers.batch_normalization(inputs=conv,name='bn_'+dname+'_'+conv_name)
                    relu = tf.nn.relu(bn,name='relu_'+dname+'_'+conv_name)
                    print('***** Feature shape: '+str(relu.get_shape()))
...
def create_single_output(model_output,
    ...
    if num_classes>1:
        ...
        return tf.expand_dims(tf.argmax(model_output, axis=dim),dim=dim)
...
class UNet3DModel(bisbasesegmentationmodel.BaseSegmentationModel):

    allowed_bridge_modes = [ 'concat','fuse','none' ]

    def __init__(self):
        self.params['name']='U-Net'
        self.params['model_dim']=3
    ...
    def get_description(self):
        return "Dilated U-Net Model for 3D images."
    ...
    def create_inference(self,training=True):
        ...
        print('===== Creating U-Net (3D) Inference Model ')
        print('=====')
